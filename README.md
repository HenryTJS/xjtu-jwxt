...
| 编号 | 功能 | 说明 | 是否需要输入参数 |
|------|------|------|----------------|
| 1 | **查询整体培养方案列表** | 按年级、方案状态等条件查询，自动翻页合并输出 | 年级代码（示例：2025） |
| 2 | **查询具体培养方案课程** | 需提供 PYFADM 加密参数 | PYFADM（示例：03406d85277248e69fd1b9cbcb261e25） |
| 3 | **查询整体课表列表** | 按学期、任务状态等条件查询，自动翻页合并输出 | 学期代码（示例：2025-2026-3） |
| 4 | **查询具体教学班课表详情** | 需提供教学班 ID | 学期代码 + 教学班ID |
| 5 | **查询课程列表** | 整体课程查询，自动翻页合并输出 | 无需输入参数 |
| 6 | **查询课程详情** | 根据课程号查询课程详细信息 | 课程号（示例：JAPN540912） |
...
| `*.json` | 调试用 JSON 文件 | ❌ 否（已在 `.gitignore`） |
